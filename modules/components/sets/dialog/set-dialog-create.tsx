...
export function SetDialogCreate({
  open,
  onOpenChange,
  onSubmit,
  isLoading = false,
}: SetDialogCreateProps) {
  const {
    formData,
    errors,
    resetForm,
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-heading-3">Create New Set</DialogTitle>
          <DialogDescription>
          ...
          <div className="space-y-3">
            ...
            >
              {Array.from({ length: 5 }).map((_, index) => {
                ...
                return (
                  <ToggleGroupItem key={value} value={value} className="font-mono w-16 border">
                    {value}
                  </ToggleGroupItem>
                );
              })}
...
export type { SetDialogCreateProps };
